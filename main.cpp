...
void launch(Environment env, double D, float fuel, float TMass, double RMFR, double EV);
//void display(Environment env, double D, float fuel, double V, int T);

void display(Environment env, double D, float fuel, double V, int T, int &flame) {
    ...
    if (D < 10000) {
        ...
        std::cout << "drag: " << env.calculateDrag(0.3, 5.2) << "N" << std::endl;
    }
    else {
...
void countdown(Environment env) {
    double EV = env.calculateEscapeV();
    double D = 70;
    float fuel = 395700;
    env.setRFuel(fuel);
    env.setRMass( 153354);
    env.setTMass();
    double RMFR = 0.0;
    float TMass = env.getTMass();
    for (int i = 3; i  != -1; i--) {
        system("clear");
        std::cout << "launch in T-" << i << std::endl;
        sleep(1);
    }
    launch(env, D, fuel, TMass, RMFR, EV);
}

void launch(Environment env, double D, float fuel, float TMass, double RMFR, double EV) {


    env.setThrust(8227000);
    int flame = 0;
